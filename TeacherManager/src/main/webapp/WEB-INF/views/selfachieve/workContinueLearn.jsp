...
<fieldset>
    <legend>参加继续教育情况</legend>
   <div class="container">
   	 <form class="form-inline" action="<%=request.getContextPath()%>/workContinueLearn/save" method="POST">
		...
		<div class="row">
			...
			<div class="span4">
				<div class="control-group">
					<label class="control-label" for="contentAndScore">内容成绩</label>
					<div class="controls">
						<input type="text" name="contentAndScore" placeholder="学习内容和成绩，不超过40字" id="contentAndScore">
					</div>
				</div>
			</div>
			<div class="span4">
				<div class="control-group">
					<label class="control-label" for="org">主办单位</label>
					<div class="controls">
						<input type="text" name="org" placeholder="主办单位" id="org">
					</div>
				</div>
			</div>
		</div>
    ...
    <c:forEach items="${data}" var="exp">
		   <tr class="${exp.id}">
		   	...
		   	<td class="contentAndScore">${exp.contentAndScore}</td>
		   	...
		   	<td><a class="btn" href="<%=request.getContextPath()%>/workContinueLearn/delete/${exp.teacherid}/${exp.id}">删除</a></td>
		   </tr> 
    </c:forEach>
    </tbody>
	</table>
