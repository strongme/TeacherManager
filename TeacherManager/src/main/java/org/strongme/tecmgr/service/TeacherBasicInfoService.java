...
public class TeacherBasicInfoService {
	...
	public Integer save(TeacherBasicInfoBean bean) {
		...
		int result = -1;
		String sql = "INSERT INTO teacherbasicinfo(teacherId,gender,native,birth,nation,politicsStatus,subject,highestDegree,professionalTitle,position,phone,email,qq,departmentName,talent,photo,remark2,honourDetail) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
		result = jdbcTemplate.execute(sql, new AbstractLobCreatingPreparedStatementCallback(this.lobHandler) {
			protected void setValues(PreparedStatement ps, LobCreator lc)
				...
				try {
					...
					lc.setBlobAsBytes(ps, 18, tmp.getHonourDetail().getBytes("UTF-8"));
	...
	public Integer update(TeacherBasicInfoBean bean) {
		final TeacherBasicInfoBean tmp = bean;
		int result = -1;
		String sql = "UPDATE teacherbasicinfo SET gender = ?,native = ?,birth = ?,nation = ?,politicsStatus =?,subject = ?,highestDegree = ?,professionalTitle = ?,position = ?,phone = ?,email = ?,qq = ?,departmentName = ?,talent = ?,photo = ?,remark2 = ?,honourDetail=? where teacherid='"+bean.getTeacherId()+"'";
		result = jdbcTemplate.execute(sql, new AbstractLobCreatingPreparedStatementCallback(this.lobHandler) {
			protected void setValues(PreparedStatement ps, LobCreator lc)
					throws SQLException, DataAccessException {
				...
				try {
					...
					lc.setBlobAsBytes(ps, 17, tmp.getHonourDetail().getBytes("UTF-8"));
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
			}});
	...
	public TeacherBasicInfoBean get(String teacherid) {
		...
		String sql = "SELECT a.id,a.teacherId,a.gender,a.native,a.birth,a.nation,a.politicsStatus,a.subject,a.highestDegree,a.professionalTitle,a.position,a.phone,a.email,a.qq,a.departmentName,a.talent,a.photo,a.remark2,a.honourDetail,b.teachername FROM teacherbasicinfo a inner join teacherrecord b on a.teacherId='"+teacherid+"' and b.teacherid='"+teacherid+"'";
		List<TeacherBasicInfoBean> list = jdbcTemplate.query(sql, new RowMapper<TeacherBasicInfoBean>() {
			public TeacherBasicInfoBean mapRow(ResultSet arg0, int arg1) throws SQLException {
				...
				result.setTeacherName(arg0.getString(20));
				try {
					...
					result.setHonourDetail(new String(lobHandler.getBlobAsBytes(arg0, 19),"UTF-8"));
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
				return result;
